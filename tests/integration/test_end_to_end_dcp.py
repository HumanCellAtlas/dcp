...
class TestEndToEndDCP(unittest.TestCase):
    ...
    def expected_results_bundle_files(self, primary_bundle_uuid, analysis_results_files_regexes):
        ...
        expected_result_file_schemas_regexes = [re.compile(f"^analysis_file_{re.escape(str(num))}\.json")
                                               for num in range(len(analysis_results_files_regexes))]

        # Metadata schema requires a new analysis_process between runs
        expected_analysis_process_regex = re.compile(f"^analysis_process_0\.json$")

        expected_files_regexes += expected_result_file_schemas_regexes
        expected_files_regexes += expected_analysis_process_regex
